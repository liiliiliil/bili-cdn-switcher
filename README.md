...
2026-07-23 已在真实 Chromium 会话中完成多轮 4K 两阶段测速、持续播放、自动恢复和随机进度跳转回归。v1.6.0 进一步把自动复测限制在有真实媒体交换的活动播放页，并加入跨标签页互斥；一个既有样本、两个低热度长视频样本和两个动态页高分辨率样本均通过播放与未缓存位置跳转测试，自动恢复 0 次。

匿名化的聚合数据、播放采样和 v1.5.1–v1.6.0 修复说明见 [TEST-REPORT-2026-07-23.md](TEST-REPORT-2026-07-23.md)。公开报告不包含测试视频 ID、标题、UP 主、账号标识、个人空间或完整媒体 URL。

## B站“4K”是否真 4K

浏览器实测同时发现了多类 B 站“4K”：真正输出 3840×2160 的视频、按钮显示 4K 但实际为 2560×1600 或 3456×2160 的视频，以及虽输出 3840×2160、播放码率却只有约 2.36 Mbps 的强压缩 AV1 视频。分辨率标签只说明播放档位，不能保证源素材具有原生 4K 细节。

逐条数据、与同源 YouTube 视频的有限对照及判断方法见 [4K-QUALITY-REPORT-2026-07-23.md](4K-QUALITY-REPORT-2026-07-23.md)。
