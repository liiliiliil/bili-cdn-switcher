...
当前开发版本：**v1.8.4**。正式包可从 GitHub Releases 下载；Chrome Web Store 首次上架正在审核，商店上线前仍可用开发者模式安装。
...
- 先快速初筛，再持续复测较快的候选。
- 自动选择结果最好的节点，也支持手动切换。
- 可以暂时禁用表现不佳的候选；节点和历史成绩仍保留，之后可一键启用。
- 缓冲持续下降且即将耗尽时，会保守地提前尝试下一个可用候选；若没有提前命中，原有的卡顿恢复仍会兜底。
...
- “禁用”只是不再把该 host 用作测速和切换目标；若它是 B 站当前原始签发节点，扩展不会硬拦截原始播放请求。
